...
clearbit_domains <- clearbit_stocks %>%
  mutate(
    domainAliases=ifelse(emailProvider==TRUE & type == "public", paste(""), domainAliases), # dissasociate with corp addresses
    domain=ifelse(emailProvider==TRUE & type == "public", host_looked_up, domain),
...
clearbit_domains <- clearbit_domains %>% 
  mutate(type=ifelse(domain=="github.com", "github", type))
...
```{r clearbit-fix-domains, eval=FALSE}
clearbit <- readRDS("dl-frameworks_domain_info_clean.Rds")
...
clearbit_unroll <- clearbit %>%
  group_by(domain) %>% 
  mutate(domainAliases = ifelse( # add the looked up host and the main domain to the aliases for later joining
    str_detect(domainAliases, host_looked_up) | host_looked_up == domain,
    domainAliases, # leave it alone
    ifelse(domainAliases=="", paste(host_looked_up, domain, sep=","), # add additional hosts to aliases
           paste(domainAliases, host_looked_up, domain, sep=","))
  )) %>% 
  summarise(domainAliases=paste(domainAliases, collapse=",")) %>%
  mutate(domainAliases=str_replace_all(domainAliases, ",,", ",")) %>% # replace double commas, TODO - fix root cause
  unnest(domainAlias=str_split(domainAliases, ",")) %>%
  mutate(domainAlias=ifelse(domainAlias=="NA", "", domainAlias)) %>%
  select(domain, domainAlias) %>%
  unique()

clearbit_domains_lookup <- clearbit %>%
  select(id, name, emailProvider, type, host_looked_up, domain) %>%
  left_join(clearbit_unroll %>% 
              select(domain, domainAlias) %>% rename(domain_main=domain), 
            by=c("host_looked_up"="domainAlias")) %>%
  unique()

# Fix types for email providers
  # make 2 seperate domain listings for email providers
# find domains with multiple entries
# filter has_email_provider

# Current (host_looked_up, domain, main_domain)
# 999e33a7-1ecb-4408-b819-5538ad718f63 Apple TRUE public me.com apple.com apple.com
# Fixed
# Apple Email TRUE personal me.com, me.com, me.com <- most frequent host_looked_up becomes the defacto domain
# add "email" to company name, make defacto domain the most frequent host_looked_up

# TODO Gmail isn't Google!
# if alias already exists as a domain entry, it probably shouldn't be added? not sure...

```


```{r link_domains_with_commits, eval=FALSE, warning=FALSE}
...
Use the email from the latest commit.
...
gitlog_networks_feb <- gitlog_networks_org %>% 
  ...
  mutate(latest_email = author_email[which.max(commit_date)],
         ...
         #author_latest_company_type = ifelse(is.na(author_latest_company_type), "No Domain Info", author_latest_company_type)) 
...
author_company_summary_has_commit <- gitlog_networks_feb %>%
  ...
  summarise(has_commit = TRUE, 
            author_company_employees=first(author_latest_company_employees),
            author_latest_company_type = first(author_latest_company_type)
            )
...
ggplot(author_company_summary_has_commit %>% filter(! is.na(author_latest_company)),
       aes(x=repo_short, y=has_commit)) +
  geom_bar(aes(fill=author_latest_company_type), stat="identity") +
  guides(fill=guide_legend(title="Company", ncol=1)) +
  coord_flip() +
  theme_few() +
  labs(x="Repository", y="Company", title="Companies with Affiliated Commits - 2018 Jan + Feb")
...
ggplot(author_company_summary_has_commit %>% 
         filter(author_latest_company_type %in% c("public")), 
       aes(x=repo_short, y=has_commit)) +
  geom_bar(aes(fill=author_latest_company), stat="identity") +
  guides(fill=guide_legend(title="Company", ncol=1)) +
  coord_flip() +
  theme_few() +
  labs(x="Repository", y="Company", title="Public Companies with Affiliated Commits - 2018 Jan + Feb")

ggplot(author_company_summary_has_commit %>% 
         filter(author_latest_company_type %in% c("private") & author_company_employees > 1), 
       aes(x=repo_short, y=has_commit)) +
  geom_bar(aes(fill=author_latest_company), stat="identity") +
  guides(fill=guide_legend(title="Company", ncol=1)) +
  coord_flip() +
  theme_few() +
  labs(x="Repository", y="Company", title="Private Companies with Affiliated Commits - 2018 Jan + Feb")

ggplot(author_company_summary_has_commit %>% 
         filter(author_latest_company_type %in% c("education")), 
       aes(x=repo_short, y=has_commit)) +
  geom_bar(aes(fill=author_latest_company), stat="identity") +
  guides(fill=guide_legend(title="Company", ncol=1)) +
  coord_flip() +
  theme_few() +
  labs(x="Repository", y="Company", title="Education Institutions with Affiliated Commits - 2018 Jan + Feb")
```
