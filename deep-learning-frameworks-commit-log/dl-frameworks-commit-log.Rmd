...
clearbit_domainAliases <- clearbit %>%
  ...
  select(domain, domain_lookup) %>%
  unique()
  

clearbit_domains_lookup <- clearbit %>%
  select(id, name, emailProvider, type, host_looked_up, domain) %>%
  mutate(host_looked_up_domain = suffix_extract(host_looked_up)$domain) %>%
  left_join(clearbit_domainAliases %>% rename(domain_main=domain),
            by=c("host_looked_up_domain"="domain_lookup")) %>%
  unique()

# some domains didn't have aliases, so just set domain_main to domain
clearbit_domains_lookup <- clearbit_domains_lookup %>%
  mutate(type=ifelse(is.na(domain), "no domain record", type),
         domain=ifelse(is.na(domain), host_looked_up, domain),
         domain_main=ifelse(is.na(domain_main), domain, domain_main))

clearbit_domains <- clearbit %>%
  left_join(clearbit_domains_lookup, by = c("id", "name", "emailProvider", "host_looked_up"))

# consolidate join
clearbit_domains <- clearbit_domains %>%
  mutate(domain=ifelse(is.na(domain.x), domain.y, domain.x),
         type=ifelse(is.na(type.x), type.y, type.x)) %>%
  select(-type.x, -type.y, -domain.x, -domain.y)

write_rds(clearbit_domains, "dl-frameworks_domain_info_clean_fixed.Rds")
```


# Author Email Consolidation

Either use an email identified as having a specific domain type or use the latest one found.
...
ggplot(author_company_summary_has_commit,
  ...
  labs(x="Repository", y="Company Type has Commit", title="Company Types with Affiliated Commits - 2018 Jan + Feb")
